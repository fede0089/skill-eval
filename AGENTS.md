...
- Both `trigger` and `functional` commands require `--workspace <path>` and `--skill <path>`, and accept `--trials <number>` (default: 5), `--agents <number>` (default: 4), `--executor-agent <name>` (default: `gemini-cli`) and `--output <path>` (default: `~/.skill-eval`, always outside the workspace and the skill). `functional` also accepts `--judge-agent <name>`, which defaults to the executor agent; `trigger` has no judge, since it grades programmatically. There is no positional agent argument.
