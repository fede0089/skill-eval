...
export async function functionalCommand(
  ...
  try {
    renderRunHeader({ command: 'functional', skillName: skill_name, executorAgent, judgeAgent, tasks: tasks.length, trials: numTrials, maxAgents, timeoutMs, runDir, evalId, evalFile });
    Logger.write(`──────────────────────────────────────────────────\n`);
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      ui.addTask({
        ...
        task: async (uiCtx, multi) => {
          ...
          const variantsTrials = await Promise.all(
          ...
          if (withoutSkillRunner && woPassedCount === woTrials.length) baselineTasksAllPassedCount++;
          ...
          const taskResult: TaskResult = {
            taskId: task.id,
            prompt: task.prompt,
            baselineTrials: withoutSkillRunner ? woTrials.map(t => ({ ...t, transcript: undefined as any })) : [],
